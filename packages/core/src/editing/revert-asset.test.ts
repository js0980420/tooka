...
describe('findAssetUsages', () => {
  it('returns 0 when the import is absent', () => {
    const src = [
      "import { ImagePlaceholder } from '@tooka/core';",
...
describe('applyRevertAsset', () => {
  ...
  it('adds ImagePlaceholder to an existing @tooka/core named import', () => {
    const src = [
    ...
    const r = applyRevertAsset(src, './assets/hero.png');
    if (!r.ok) throw new Error(`expected ok, got ${r.error}`);
    expect(r.source).toMatch(/import \{ type Page, ImagePlaceholder \} from '@tooka\/core';/);
  });

  it('adds a separate value import when the only @tooka/core import is type-only', () => {
    const src = [
    ...
    expect(r.source).toContain("import { ImagePlaceholder } from '@tooka/core';");
  });

  it('adds a fresh @tooka/core import when none exists', () => {
    const src = [
      "import hero from './assets/hero.png';",
      'export default [() => (',
      "  <img src={hero} alt='x' style={{ objectFit: 'cover' }} />",
    ...
    const r = applyRevertAsset(src, './assets/hero.png');
    if (!r.ok) throw new Error(`expected ok, got ${r.error}`);
    expect(r.source.split('\n')[0]).toBe("import { ImagePlaceholder } from '@tooka/core';");
  });
