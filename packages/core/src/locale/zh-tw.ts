...
export const zhTW: Locale = {
  ...
  home: {
    ...
    noSlidesYet: '尚無卡片',
    draftEmptyTitle: '尚無草稿',
    emptyTemplatesHint: '從模板挑一個風格，按「加入草稿」開始建立。',
    goToTemplates: '前往模板頁',
  ...
  connects: {
    ...
    imgbbTitle: 'Imgbb 圖床',
    imgbbDesc:
      '發佈時圖卡會先上傳到 Imgbb 產生公開圖片網址，Meta API 才能讀取並顯示圖片；尚未連接前，自動發佈的圖片將無法顯示。',
    imgbbRegisterLink: '前往 Imgbb 免費註冊並取得 API Key',
    imgbbKeyLabel: 'Imgbb API Key',
    imgbbKeyPlaceholder: '貼上你的 API Key',
    imgbbKeyStatus: 'API Key 狀態',
    imgbbSavedKeyLabel: '已儲存的 API Key',
    ...
    toastImgbbInvalidKey: 'Imgbb API Key 無效，請確認已完整複製。',
    toastImgbbConnectionSuccess: '連線成功！Imgbb 圖床可以正常上傳。',
    toastImgbbTestFailed: '連線測試失敗，請檢查 Imgbb API Key。',
    confirmImgbbDisconnect: '確定要斷開 Imgbb 圖床連線並刪除 API Key 嗎？',
    ...
    imgbbTutorialLink: '如何取得 Imgbb API Key',
  },
  ...
  publish: {
    ...
    toastExternalLinks: '{platforms} 文案包含外部連結，請移除後再發布。',
    toastInvalidInstagramHashtags: 'Instagram 必須填寫正好 5 個不重複的標籤，正文請勿另加 #。',
    toastInvalidTopicTag: 'Threads 必須填寫 1 個主題標籤，且不能包含 #、句點（.）或 &。',
    toastInstagramCardLimit: 'Instagram 透過 API 發布最多支援 10 張圖卡。',
    toastAnalyzingCard: '正在讀取圖卡內容並整理文案…',
    toastCaptionsGenerated: '已找出 Hook、重點與 CTA，並產生三平台文案。',
    toastAnalyzeFailed: '無法分析圖卡內容：{error}',
    ...
    stepGenerateText: '2. 圖卡文案快速套用',
    generateTextHint: '依序讀取圖卡文字，找出 Hook、重點與 CTA，再改寫成三平台格式。',
    analyzingCard: '分析圖卡中…',
    generateCaptions: '分析並套用三平台',
    regenerateCaptions: '重新分析並套用',
    analysisEmpty: '選擇圖卡後開始分析，三平台仍可各自編輯。',
    insightHook: 'Hook',
    insightKeyPoints: '重點',
    insightCta: 'CTA',
    insightPage: '第 {page} 張',
    insightCount: '共 {count} 個重點',
    insightGenerated: '未找到明確 CTA，已補上建議',
    ...
    igHashtagsLabel: 'Hashtags（必填 5 個，將自動附加於文案下方）',
    igHashtagPlaceholder: '標籤 {n}',
    threadsTopicTagPlaceholder: '主題標籤（必填，不含 #，每篇限一個）',
  ...
  prompts: {
    ...
    tipsTitle: '💡 使用技巧：',
    tipsBody:
      '先在模板頁按「加入草稿」拿到一組輪播，再點擊右上角的複製按鈕，把提示詞貼給 AI 助理並換掉其中的主題文字（例如輪播主題、標題、主色調等），就能快速把草稿調整成你的內容。',
    emptyState: '目前尚未儲存任何提示詞。點擊右上角「新增提示詞」開始吧！',
    sampleCategory: '模板調整實例',
    sample1Title: '整組輪播：換成我的主題',
    sample1Content:
      '我剛從模板加入一組草稿輪播，把主題換成「◯◯◯」：版式、配色與字體都不變，只改文字。第一張 Hook 要點出目標受眾的痛點，中間每張卡只講一個重點，最後一張收在「留言關鍵字索取」的 CTA。',
    sample2Title: '單張卡：改標題與副標',
    sample2Content: '把這張卡的大標題改成「◯◯◯」，副標題改成「◯◯◯」，其他版面與樣式都不要動。',
    sample3Title: '整組輪播：調整視覺風格',
    sample3Content:
      '把這組輪播的主色調改成暖米色系：背景 #F8F2E6、文字深棕、強調色橄欖綠，標題字體換成明朝體（Noto Serif TC）。整組卡片一起改，版式保持不變。',
    sample4Title: '插入一張大數字卡',
    sample4Content:
      '在第 3 張後面加一張「大數字卡」：用一個大數字呈現「◯◯」，數字下方一句說明，版式與配色沿用這組輪播的風格。',
    sample5Title: '換圖片與帳號資訊',
    sample5Content:
      '把封面卡的圖片佔位換成我拖進素材庫的「◯◯.png」，並把每張卡的帳號 handle 改成「@my.account」。',
  },
  tutorials: {
    pageTitle: '教學指南與手冊',
    categoryLabel: '文章分類',
